...
async def run_experiment(experiments_dic: Dict[str, Dict[str, Any]]):
    ...
    # Look for the most recent output file matching baseName_output_*.xlsx
        matching_files = glob.glob(f"{base_name}_output_*.xlsx")
        if not matching_files:
            raise FileNotFoundError(f"No output file found for experiment {experiment_label}")
    # Pick the newest file by creation time
        new_file_path = max(matching_files, key=os.path.getctime)
    # Now safely color it
        color_cells_in_excel(
            new_file_path,
